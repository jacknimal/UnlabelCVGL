...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False, logger=None):
    end = time.time()
    if print_flag:
...
def compute_modal_invariant_jaccard_distance(target_features, file, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False, logger=None):
    end = time.time()
    all_file_name=[]
    for i, (fname, _) in enumerate(file):
    ...
    if k2 != 1:
        ...
        for i in range(N):
            ...
            for ii in initial_rank[i,:k2]:
                ...
                if 'satellite' in all_file_name[ii]:
                    feas_NIR_temp.append(V[ii,:])
                elif 'drone' in all_file_name[ii]:
